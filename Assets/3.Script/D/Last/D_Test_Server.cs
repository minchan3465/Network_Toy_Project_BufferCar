...
public class D_Test_Server : MonoBehaviour {
    ...
    private void Awake() {
        ...
        if (!File.Exists(Path + "/License2.json")) {
            Default_data(Path);
        }
        Path = Path + "/License2.json";
    ...
    private void Default_data(string path) {
        ...
        File.WriteAllText(path + "/License2.json", data.ToString());
    }
    ...
    private void OnApplicationQuit() {//프로그램이 꺼졌을 때
